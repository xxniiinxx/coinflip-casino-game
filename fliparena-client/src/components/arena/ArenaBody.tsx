...
import { OpenMatchModal } from "../Modals/OpenMatchModal"
import { useWallet } from "@solana/wallet-adapter-react"
import { errorAlert } from '../ToastGroup';
import { useArenaSocket } from "../../providers/ArenaSocketProvider";
import { AcceptMatchModal } from "../Modals/AcceptMatchModal";
...
import { MESSAGES_PER_PAGE, RPC } from "../../lib/constant";
import ConfettiExplosion from 'react-confetti-explosion';
import { useArenaEffects } from "../../providers/ArenaEffectsProvider";

export const ArenaBody = () => {
    ...
    const { loadChat, matches, hasMore, setOpeningState, isNew, isScroll, walletStats, chatMessages, sendChat } = useArenaSocket();
    const { confetti, setConfetti } = useArenaEffects()
    ...
    useEffect(() => {
        if (isScroll) {
            if (messageRef.current && chatMessages.length % MESSAGES_PER_PAGE === 0) {
                messageRef.current.scrollTop = 100;
            }
        } else {
            if (chatMessages.length > 0 && chatMessages[chatMessages.length - 1].wallet === publicKey?.toBase58() || !isNew) {
                scrollToElement();
            }
        }
    ...
    const submit = async () => {
        try {
            if (text !== "" && !pending) {
                ...
                sendChat(message.wallet, message.message);
                setText("")
                setPending(true);
    ...
    return <div className='px-3 md:px-4 xl:px-9 py-3 md:py-4 bg-bg-light flex flex-col-reverse md:flex-row justify-between gap-3 mt-[60px] h-full md:h-[calc(100vh-60px)] overflow-auto relative'>
                ...
                <div className="flex flex-col items-center text-white px-[14px] py-[18px] gap-3">
                    ...
                    <div className="flex justify-between w-full gap-2">
                        <div className="w-1/4 border border-white/10 rounded-2xl p-2">
                            ...
                            <p className="text-md text-white">{walletStats.totalMatches}</p>
                        </div>
                        <div className="w-1/4 border border-white/10 rounded-2xl p-2">
                            <p className="text-[12px] text-grey">Wins</p>
                            <p className="text-md text-white">{walletStats.wins}</p>
                        </div>
                        <div className="w-1/4 border border-white/10 rounded-2xl p-2">
                            <p className="text-[12px] text-grey">Losses</p>
                            <p className="text-md text-white">{walletStats.totalMatches - walletStats.wins}</p>
                        </div>
                        <div className="w-1/4 border border-white/10 rounded-2xl p-2">
                            <p className="text-[12px] text-grey">Win Rate</p>
                            <p className="text-md text-white">{walletStats.totalMatches > 0 ? (walletStats.wins / walletStats.totalMatches * 100).toFixed(1) : 0}%</p>
                        </div>
                    </div>
                    <button className="rounded-lg border border-purple w-full py-2 text-purple text-xm text-center hover:border-white hover:text-white transition" onClick={() => create_game()}>Open Match</button>
                </div>
            </div>
        ...
        <OpenMatchModal open={createModal} handleModal={handleModal} />
        <AcceptMatchModal open={joinModal} handleModal={handleJoin} index={dataIndex} />
    </div>
}