...
import { token_list } from "../../lib/menu";
...
export const OpenMatchModal = (props: { open: boolean, handleModal: any }) => {
    const { openMatch, opening, setOpeningState } = useArenaSocket();
        ...
        <div className="w-full h-fit md:w-fit border border-white/10 rounded-2xl relative bg-bg-light flex flex-col shadow-2xl mt-20">
            ...
            <div className={`h-full flex flex-col ${step === 1?`gap-6 sm:gap-16 mt-16 sm:mt-28`:`gap-6 mt-20`}`}>
                ...
                <div className="w-full flex gap-3 text-grey border border-t-white/10 border-b-0 border-x-0 py-[16px] px-[20px]">
                    ...
                    <button className="rounded-lg border border-white/10 w-1/2 mx-auto py-2 text-xm text-center hover:border-purple hover:bg-purple hover:text-white transition" onClick={() => handleContinue()}>{step == 1 ? "Open Match" : "Continue"}</button>
                </div>
            </div>
            {opening? <div className="w-full h-full absolute top-0 left-0 backdrop-blur-sm flex justify-center items-center z-30">
            </div>: <></>}
        </div>
    </div>
}
