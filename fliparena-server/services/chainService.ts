...
import { GLOBAL_VAULT_SEED, GAME_VAULT_SEED, FLIPARENA_PROGRAM } from "../constants";
import { getOrCreateATAInstruction, getAssociatedTokenAccount } from "./tokenUtils";
import {
...
let programId = new anchor.web3.PublicKey(FLIPARENA_PROGRAM);
/**
...
export const initializeGlobalVault = async () => {
...
export const openMatch = async (
  ...
  try {
    ...
    let search = await MatchModel.findOne({ matchPda: newGame.toBase58() });
    while (search) {
      index++;
      newGame = PublicKey.findProgramAddressSync(
        [
      ...
      search = await MatchModel.findOne({ matchPda: newGame.toBase58() });
    }
...
export const acceptMatch = async (
  ...
  try {
    ...
    const base64Transaction = Buffer.from(serializedTx).toString("base64");
    return { base64Transaction, matchPda };
  } catch (error) {
    console.log(error);
    return null;
  }
...
export const fetchMatchStatus = async (matchPdaAddress: String) => {
  const matchPda = new PublicKey(matchPdaAddress);
...
export const settleMatch = async (
  ...
  try {
    ...
    const creatorAta = getAssociatedTokenAccount(tokenMint, creator);
    const playerAta = getAssociatedTokenAccount(tokenMint, player);
    const random = randomnessAccountAddress(matchPda.toBuffer());
...
export const fetchGlobalVaultStatus = async () => {
  const global = PublicKey.findProgramAddressSync(
    [Buffer.from(GLOBAL_VAULT_SEED)],
    FLIPARENA_PROGRAM
  )[0];
